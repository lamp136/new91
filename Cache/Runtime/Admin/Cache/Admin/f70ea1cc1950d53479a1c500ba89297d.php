                                    ...
                                    <table class="table table-striped table-bordered" id="sample_1">
                                        <thead>
                                            <tr>
                                                ...
                                                <th width="8%" class="hidden-phone">姓名</th>
                                                <th width="8%" class="hidden-phone">预约时间</th>
                                                <th width="13%" class="hidden-phone">商家名称</th>
                                                <th width="8%" class="hidden-phone">下单时间</th>
                                                <th width="8%" class="hidden-phone">佣金实收金额</th>
                                                <th width="8%" class="hidden-phone">返现金额</th>
                                                <th width="8%" class="hidden-phone">完成时间</th>
                                                <th width="12%" class="hidden-phone">跟踪信息</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="odd gradeX">
                                                ...
                                                <td><?php echo ($vo["return_fact_money"]); ?></td>
                                                <td><?php echo (date('Y-m-d',$vo["updated_time"])); ?></td>
                                            <td>

                                                <?php if($vo['brokerage_status'] == C('BROKERAGE_STATUS.SUCCESS')): if (showHandle('OrderGrave/orderbillinfo')) { ?>
                                                 <a class="btn btn-small btn-success viewBill"  href="javascript:;" data-id="<?php echo ($vo["id"]); ?>" ><i class="icon-eye-open icon-white"> </i>查看票据</a>
                                                 <?php } endif; ?>
   ...
   <!--查看票据开始-->
    ...
    <script type="text/javascript">
        ...
        $('.viewBill').click(function(){
            ...
            $.ajax({
                url: "<?php echo U('OrderGrave/orderbillinfo');?>",
                type: 'POST',
                data: {'orderId':orderId},
                success:function(d){
                    var result = eval("(" + d + ")");
