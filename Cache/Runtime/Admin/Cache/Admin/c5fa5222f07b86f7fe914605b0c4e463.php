            ...
            <div class="row-fluid">
                <div class="span12">
                    ...
                    <ul class="breadcrumb">
                        ...
                        <li><a href="javascript:void(0)"><?php echo ($storeInfo["store_name"]); ?>新闻列表</a><span class="divider-last">&nbsp;</span></li>
                    ...
                    <div class="span12">
                        ...
                        <div class="widget">
                            <div class="widget-title">
                                ...
                                <?php if (showHandle('BaseStore/addNew')) { ?>
                                    <span class="tools">
                                        <a href="<?php echo U('BaseStore/addNew',array('sid'=>$storeId,'np'=>$np,'sname'=>$sname,'sprovince'=>$sprovince));?>" class="icon-plus">添加商家新闻</a>
                                    </span>
                                 <?php } ?>
                            </div>
                                ...
                                <table class="table table-striped table-bordered" id="sample_1">
                                    ...
                                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="odd gradeX">
                                            <td><?php echo ($vo["title"]); ?>
                                                <?php if(($vo["top"]) == "1"): ?><img width="19px" src="<?php echo (IMG_URL); ?>top.png" title="置顶"><?php endif; ?>
                                                <?php if(($vo["is_hot"]) == "1"): ?><img width="21px" src="<?php echo (IMG_URL); ?>hot.png" title="热门"><?php endif; ?>
                                                <?php if(($vo["recommend"]) == "1"): ?><img width="23px" src="<?php echo (IMG_URL); ?>recommend.png" title="推荐"><?php endif; ?>
                                            </td>
                                            <td><?php echo ($vo["cateName"]); ?></td>
                                            <td>
                                                <?php if($vo["status"] == 1): ?>已发布
                                                    <?php elseif($vo["status"] == 0): ?>审核中
                                                    <?php elseif($vo["status"] == -1): ?>删除<?php endif; ?>
                                            </td>
                                            <td><?php echo (date("Y-m-d H:i:s",$vo["published_time"])); ?></td>
                                            <td><?php echo (date("Y-m-d H:i:s",$vo["created_time"])); ?></td>
                                            <td>
                                                <?php if (showHandle('BaseStore/editNew')) { ?>
                                                    <a href="<?php echo U('BaseStore/editNew',array('sid'=>$vo['store_id'],'nid'=>$vo['id'],'np'=>$np,'sname'=>$sname,'sprovince'=>$sprovince));?>" class="btn btn-primary btn-small">
                                                       <i class="icon-pencil icon-white"></i> 编辑
                                                    </a>
                                                <?php } if (showHandle('BaseStore/deleteNew')) { ?>
                                                     <?php if($vo['status'] != -1): ?><button type="button" class="btn btn-danger btn-small del" data-id="<?php echo ($vo["id"]); ?>" data-status="<?php echo (C("DELETE_STATUS")); ?>">
                                                            <i class="icon-remove  icon-white"> </i>删除
                                                        </button>
                                                        <?php else: ?>
                                                        <a href="javascript:void(0)" class="btn btn-success btn-small start" data-id="<?php echo ($vo["id"]); ?>" data-status="<?php echo (C("DEFAULT_STATUS")); ?>">
                                                            <i class="icon-ok  icon-white"> </i>启用
                                                        </a><?php endif; ?>
                                                <?php } if (showHandle('BaseStore/newImage')) { ?>
                                                    <a href="<?php echo U('BaseStore/newImage',array('nid'=>$vo['id'],'sid'=>$vo['store_id'],'np'=>$np,'sname'=>$sname,'sprovince'=>$sprovince));?>" class="btn btn-info btn-small">
                                                        图片
                                                    </a>
                                                <?php } ?>
                                            </td>
