                    ...
                    <div class="span12">
                        ...
                        <div class="widget">
                            <div class="widget-body">
                                <form method="get" class="form-horizontal" autocomplete="off" action="<?php echo U('Storage/outputGoods');?>">
                                    搜索：
                                    <select class="input-medium m-wrap" tabindex="1" name="storage_name">
                                        <option value="0">--选择仓库--</option>
                                        <?php if(is_array($storage)): $i = 0; $__LIST__ = $storage;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($_GET['storage_name'] == $key): ?><option value="<?php echo ($key); ?>" selected="selected"><?php echo ($vo); ?></option>
                                            <?php else: ?>
                                                <option value="<?php echo ($key); ?>"><?php echo ($vo); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                                    </select>
                                    <input type="text" name="order_main_sn" class="input-medium" placeholder="订单号" value="<?php echo ($_GET['order_main_sn']); ?>"/>
                                ...
                                <table class="table table-striped table-bordered" id="sample_1">
                                    ...
                                    <tbody>
                                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="odd gradeX">
                                                <td><?php echo ($storage[$vo[storage_id]]); ?></td>
                                                <td><?php echo ($serviceGoods[$vo[skuid]]); ?></td>
                                                <!--<td><?php echo ($vo["running_series"]); ?></td>-->
                                                <td><?php echo ($vo["number"]); ?></td>
                                                <td><?php echo ($vo["order_main_sn"]); ?></td>
                                                <td><?php echo (date("Y-m-d H:i",$vo["delivery_time"])); ?></td>
                                                <td><?php echo ($vo["delivery_man"]); ?></td>
                                                <td><?php echo ($vo["logistics_number"]); ?></td>
                                                <!--<td><?php echo ($vo["consignee"]); ?></td>
                                                <td><?php echo ($vo["consignee_mobile"]); ?></td>
                                                <td><?php echo ($vo["shipping_address"]); ?></td>-->
                                                <td class="provincecity">
                                                    <a href="javascript:;" style="color:green;">查看</a>
                                                    <div class='hide address'>
                                                        提货人：<?php echo ($vo["take_stock_people"]); ?><br/>
                                                        手机号：<?php echo ($vo["take_stock_mobile"]); ?><br/>
                                                        提货时间：<?php echo (date("Y-m-d H:i",$vo["take_stock_time"])); ?>
                                                    </div>
                                                </td>
                                                
                                                <td>
                                                    <?php switch($vo["status"]): case "1": ?>待出库<?php break;?>
                                                        <?php case "2": ?>已出库<?php break;?>
                                                        <?php default: ?>联系开发人员<?php endswitch;?>
                                                </td>
                                                <td>
                                                    <?php if (showHandle('Storage/outputOrder')) { ?>
                                                        <?php if($vo["status"] == 1): ?><button type="button" class="btn btn-small btn-primary output" data-id="<?php echo ($vo["id"]); ?>" /><i class="icon-pencil icon-white"></i>出货<?php endif; ?>
                                                    <?php } ?>
                                                </td>
