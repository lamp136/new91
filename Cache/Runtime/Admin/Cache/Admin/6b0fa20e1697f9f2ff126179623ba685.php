                            ...
                            <div class="widget">
                                ...
                                <div class="widget-body">
                                    <div class="control-group">
                                        <div class="controls">
                                            <form action="/system.php/Comment/index.html" method="get">
                                                <select class="input-medium m-wrap" tabindex="1" name="comment_status">
                                                    <option value="0">--审核状态--</option>
                                                    <option value="<?php echo (C("COMMENT_STATUS.UNPASS")); ?>"<?php if($getMap == -1): ?>selected="selected"<?php endif; ?>>未通过</option>
                                                    <option value="<?php echo (C("COMMENT_STATUS.PASS")); ?>"<?php if($getMap == 1): ?>selected="selected"<?php endif; ?>>已通过</option>
                                                </select>  
                                    ...
                                    <table class="table table-striped table-bordered table-hover">
                                        ...
                                        <tbody>
                                        <?php if(is_array($comment)): $i = 0; $__LIST__ = $comment;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="odd gradeX">
                                                <td><?php echo ($vo["Store"]["store_name"]); ?></td>
                                                <td><?php echo ($vo["Member"]["name"]); ?></td>
                                                <td><?php echo ($vo["Member"]["mobile"]); ?></td>
                                                <td><?php echo ($vo["content"]); ?></td>
                                                <td><?php echo ($vo["environment"]); ?>分</td>
                                                <td><?php echo ($vo["service"]); ?>分</td>
                                                <td><?php echo ($vo["price"]); ?>分</td>
                                                <td><?php echo ($vo["traffic"]); ?>分</td>
                                                <td><?php echo ($vo["comment_time"]); ?></td>
                                                <td>
                                                    <?php if(showHandle('Comment/editStatus')){ ?>
                                                        <?php if($vo["comment_status"] == 1): ?><a class="btn btn-success btn-small edit" data-id="<?php echo ($vo["id"]); ?>"><i class="icon-ok  icon-white"> </i> 已通过</a>
                                                        <?php elseif($vo["comment_status"] == -1): ?>
                                                            <a class="btn btn-danger btn-small edit"  data-id="<?php echo ($vo["id"]); ?>"><i class="icon-remove  icon-white"></i> 未通过</a>
                                                        <?php else: ?>
                                                            <a class="btn btn-small btn-warning edit"  data-id="<?php echo ($vo["id"]); ?>"><i class="icon-cog  icon-white"> 审核中</i></a><?php endif; ?>
                                                    <?php } ?>
                                                </td>
                ...
                <table class="table table-bordered table-hover">
                    <tbody>
                        <tr>
                            <td>选择审核状态：</td>
                            <td>
                                <input name="comment_status" value="<?php echo (C("COMMENT_STATUS.PASS")); ?>" type="radio" data-rule="checked"> 通过审核 
                                <input name="comment_status" value="<?php echo (C("COMMENT_STATUS.UNPASS")); ?>" type="radio" data-rule="checked"> 未通过
                            </td>
                        </tr>
                    </tbody>
                </table>
