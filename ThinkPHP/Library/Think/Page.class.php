...
class Page{
    ...
    private $config  = array(
        ...
        'first'  => '第一页',
        'last'   => '...%TOTAL_PAGE%',
        'theme'  => '%HEADER% <ul> %FIRST%  %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%</ul>',
    );
    ...
    public function frontshow() {
        ...
        if(strpos($urlpath, '?')!==false){
            //判断是否有P参数
            if(isset($this->parameter['p']) && !empty($this->parameter['p'])){
                $urlpath = preg_replace("/p=.+/",'',  $urlpath);
                $this->url = $urlpath .$this->p.'='.urlencode('[PAGE]');
            }else{
                //否则不存在
                $this->url = $urlpath .'&'.$this->p.'='.urlencode('[PAGE]');
            }
